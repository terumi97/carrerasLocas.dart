...
class PadRacingGame extends Forge2DGame with KeyboardEvents {
  static const String description = '''
     Creado a partir de otros proyectos en flutter con cambios extra en colores, texturas
     y diseños de carros e interfaz con el afan de aprender mas sobre flutter y el lenguaje
     dart.
  ''';
  ...
  void start({required int numberOfPlayers}) {
    ...
    RectangleComponent viewportRimGenerator() =>
        RectangleComponent(size: viewportSize, anchor: Anchor.topLeft)
          ..paint.color = GameColors.magenta.color
...
class Ball extends BodyComponent<PadRacingGame> with ContactCallbacks {
  ...
  Future<void> onLoad() async {
    await super.onLoad();
    renderBody = false;
    _shaderPaint = GameColors.purple.paint
      ..shader = Gradient.radial(
        Offset.zero,
        radius,
        [
          GameColors.purple.color,
  ...
  Color get color {
    switch (this) {
      case GameColors.purple:
        return ColorExtension.fromRGBHexString('#bb33ff');
      case GameColors.magenta:
        return ColorExtension.fromRGBHexString('#33fff9');
    }
  }
        ...
        child: Wrap(
          children: [
            MenuCard(
              children: [
                Text(
                  'Jugador ${game.winner!.playerNumber + 1} es el ganador',
                  style: textTheme.displayLarge,
                ),
                const SizedBox(height: 10),
                Text(
                  'Tiempo: ${game.timePassed}',
...
class LapLine extends BodyComponent with ContactCallbacks {
  ...
  @override
  Body createBody() {
    paint.color = (isFinish ? GameColors.purple.color : GameColors.purple.color)
      ..withOpacity(0.5);
    paint
  ...
  Future<void> onLoad() async {
    ...
    void updateLapText() {
      if (lapNotifier.value <= PadRacingGame.numberOfLaps) {
        ...
        lapCounter.text = 'Carrera finalizada';
      }
    }
            ...
            Column(
              children: [
                MenuCard(
                  children: [
                    Text(
                      'CTR carreras de la muerte',
                      style: textTheme.displayLarge,
                    ),
                    Text(
                      'El primer jugador en completar 3 carreras gana',
                      style: textTheme.bodyLarge,
                    ),
                    const SizedBox(height: 10),
                    ElevatedButton(
                  ...
                  children: [
                    RichText(
                      text: TextSpan(
                        children: [
                          TextSpan(
                            text: 'Juego echo y editado por:',
                            style: textTheme.bodyMedium,
                          ),
                          TextSpan(
                            text: 'Ottmar Mendoza',
                            style: textTheme.bodyMedium
                                ?.copyWith(color: GameColors.purple.color),
...
class MenuCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      color: Colors.black,
      shadowColor: GameColors.purple.color,
      elevation: 10,
